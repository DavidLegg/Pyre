...
ROUND_PATTERN = re.compile(r'Running round (\d+)')
...
def main(in_file: str):
    ...
    for ln in text.splitlines():
        if m := ROUND_PATTERN.search(ln):
            ...
            this_edit_batch.append(f'edit({translate_activity_match(m)} to {m.group("new_activity")})')
    print_batch(this_edit_batch, first_batch, incon_time)


def translate_activity_match(m):
    return f'GroundedActivity(Instant.parse("{m.group("time")}"), Name("{m.group("name")}"), {m.group("activity")})'
