...
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.*
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.Expression.*
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.Expression.Companion.andThen
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.StatementBlock.*
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.StatementBlock.EffectBlock.CounterEffectBlock.IncrementCounter
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.StatementBlock.EffectBlock.CounterEffectBlock.SetCounter
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.StatementBlock.EffectBlock.SlopeEffectBlock.IncreaseSlope
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.StatementBlock.EffectBlock.SlopeEffectBlock.SetSlope
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.StatementBlock.EffectBlock.SwitchEffectBlock.SetSwitch
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.StatementBlock.EffectBlock.SwitchEffectBlock.ToggleSwitch
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.StatementBlock.EffectBlock.TimerEffectBlock.*
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.StatementBlock.ReportBlock.*
import gov.nasa.jpl.parakeet.foundation.incremental.BlockTestModel.StatementBlock.SaveValue.*
import gov.nasa.jpl.parakeet.foundation.incremental.IncrementalSimulatorOperations.add
import gov.nasa.jpl.parakeet.foundation.incremental.IncrementalSimulatorOperations.edit
import gov.nasa.jpl.parakeet.foundation.incremental.IncrementalSimulatorOperations.isEmpty
import gov.nasa.jpl.parakeet.foundation.incremental.IncrementalSimulatorOperations.minus
import gov.nasa.jpl.parakeet.foundation.incremental.IncrementalSimulatorOperations.move
import gov.nasa.jpl.parakeet.foundation.incremental.IncrementalSimulatorOperations.plus
import gov.nasa.jpl.parakeet.foundation.incremental.IncrementalSimulatorOperations.remove
import gov.nasa.jpl.parakeet.foundation.incremental.IncrementalSimulatorOperations.unaryPlus
import gov.nasa.jpl.parakeet.foundation.incremental.TestModel.*
...
import gov.nasa.jpl.parakeet.foundation.resources.discrete.BooleanResourceOperations.toggle
...
import gov.nasa.jpl.parakeet.foundation.resources.discrete.DoubleResourceOperations.minus
...
import gov.nasa.jpl.parakeet.foundation.resources.discrete.IntResourceOperations.minus
import gov.nasa.jpl.parakeet.foundation.resources.discrete.IntResourceOperations.plus
...
import gov.nasa.jpl.parakeet.foundation.resources.timer.TimerResourceOperations
import gov.nasa.jpl.parakeet.foundation.resources.timer.TimerResourceOperations.greaterThan
import gov.nasa.jpl.parakeet.foundation.resources.timer.TimerResourceOperations.minus
...
import gov.nasa.jpl.parakeet.foundation.resources.timer.TimerResourceOperations.restart
...
import gov.nasa.jpl.parakeet.general.resources.discrete.ListResourceOperations.isNotEmpty
import gov.nasa.jpl.parakeet.general.resources.discrete.ListResourceOperations.pop
import gov.nasa.jpl.parakeet.general.resources.discrete.ListResourceOperations.push
import gov.nasa.jpl.parakeet.general.resources.discrete.MutableListResource
...
import gov.nasa.jpl.parakeet.general.resources.polynomial.PolynomialResourceOperations.minus
import gov.nasa.jpl.parakeet.general.resources.polynomial.PolynomialResourceOperations.plus
import gov.nasa.jpl.parakeet.general.resources.polynomial.PolynomialResourceOperations.polynomialResource
import gov.nasa.jpl.parakeet.general.results.SimulationResults
import gov.nasa.jpl.parakeet.kernel.DependentMap.Companion.valueEquals
...
import gov.nasa.jpl.parakeet.kernel.tasks.TaskHistory.Companion.valueEquals
...
class IncrementalSimulatorTest {
    ...
    fun `polynomial precision losses interfering with condition logic`() {
        ...
        // It is fully documented at https://github.com/DavidLegg/Parakeet/issues/67. Copying that description of the problem:
    ...
    private data class BlockInstrumentation(
        ...
        val resourceExpressionSamples: MutableMap<Expression<*>, MutableList<Any?>> = mutableMapOf()
    )
    ...
    private fun <R> Expression<R>.instrument(): Expression<R> {
        ...
        return mapSubexpressions(object : ExpressionMapper {
            ...
            instrumentation.expressionResults.computeIfAbsent(this@instrument) { mutableListOf() }.add(result)
            @Suppress("UNCHECKED_CAST")
            if (result is ThinResource<*>) {
                // If the result is a resource, instrument it as well
                // We can safely assume that any ThinResource result is a standard Resource, returning some kind of dynamics.
                result as Resource<out Dynamics<*, *>>
                // From there, we can build our own resource which logs the samples to our instrumentation.
                Resource { result.getDynamics().also {
                    val value = it.data.value()
                    instrumentation.resourceExpressionSamples.computeIfAbsent(this@instrument) { mutableListOf() }.add(value)
                }} as R
            } else result
        }
    }
    ...
    private fun StatementBlock.simplifications(): Sequence<StatementBlock> = sequence {
        when (this@simplifications) {
            ...
            is IncrementCounter -> {
                yieldAll(index.indexSimplifications().map { copy(index = it) })
            ...
            is ToggleSwitch -> yieldAll(index.indexSimplifications().map { copy(index = it) })
            is PauseTimer -> yieldAll(index.indexSimplifications().map { copy(index = it) })
            is ResetTimer -> yieldAll(index.indexSimplifications().map { copy(index = it) })
            is RestartTimer -> yieldAll(index.indexSimplifications().map { copy(index = it) })
            is ResumeTimer -> yieldAll(index.indexSimplifications().map { copy(index = it) })
    ...
    @Suppress("UNCHECKED_CAST")
    context (instrumentation: Lazy<BlockInstrumentation>)
    private fun Expression<Int>.indexSimplifications(): Sequence<Expression<Int>> = sequence {
        if (this@indexSimplifications is ConstantInt && value !in 0..<BlockTestModel.BLOCK_TEST_MODEL_SIZE) {
            // For the special case of indices, consider modding a constant index down to its reduced form.
            // This doesn't change the complexity of the test, but it does make it easier to read.
            yield(ConstantInt(floorMod(value, BlockTestModel.BLOCK_TEST_MODEL_SIZE)))
        }
        yieldAll(simplifications())
    }
...
class BlockTestModel(initScope: InitScope) {
    ...
    interface Expression<R> {
        ...
        companion object {
            ...
            // Like log above, this lets us inspect and potentially modify the results of an expression.
            // This version is intended primarily for automatic simplification, though.
            fun <R> Expression<R>.andThen(block: (R) -> R): Expression<R> = object : Expression<R> {
                context(_: TaskScope)
                override fun evaluate(model: BlockTestModel, locals: BlockLocals): R =
                    block(this@andThen.evaluate(model, locals))

                override fun mapSubexpressions(f: ExpressionMapper): Expression<R> =
                    f.apply(this@andThen).andThen(block)
            }
        }
    }
}
