...
import gov.nasa.jpl.parakeet.UnitAwareAssertions.assertEquals
...
import gov.nasa.jpl.parakeet.general.units.discrete_unit_aware_resource.UnitAwareDiscreteResourceOperations.set
import gov.nasa.jpl.parakeet.general.units.unit_aware_resource.UnitAwareResourceOperations.getValue
...
class DataModelTest {
    ...
    private class TestModel(context: InitScope) {
        ...
        init {
            with (context) {
                dataRate = discreteResource("data_rate", 0.0 * BITS_PER_SECOND)
                downlinkRate = discreteResource("downlink_rate", 0.0 * BITS_PER_SECOND)
                model = DataModel(this,
                    DataModel.Config(32.0 * GIGABYTE),
                    DataModel.Inputs(dataRate.upcast(), downlinkRate.upcast()))
            }
        }
    }
